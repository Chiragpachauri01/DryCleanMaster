...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!data.email || !data.finalDate || !data.finalTime) {
    ...
    await transporter.sendMail({
      ...
      to: data.email,
      subject: `🔁 Booking Rescheduled — ${BRAND}`,
      attachments: [logoAttachment()],
      html: emailShell("Booking Rescheduled 🔁", `
        <p style="margin:0 0 8px;font-size:15px;color:#1a1a2e;font-weight:700;">Dear ${safeName},</p>
        <p style="margin:0 0 24px;font-size:14px;color:#475569;line-height:1.75;">Your booking has been <strong>rescheduled</strong>. Here are your updated appointment details:</p>
        <table width="100%" cellpadding="12" cellspacing="0" style="background:#f3f4f6;border-radius:10px;">
          <tr><td style="color:#6b7280;">🧺 Service</td><td align="right" style="font-weight:600;">${safeService}</td></tr>
          <tr><td style="color:#6b7280;">📅 New Date</td><td align="right" style="font-weight:700;color:#0f766e;">${safeDate}</td></tr>
          <tr><td style="color:#6b7280;">⏰ New Time</td><td align="right" style="font-weight:700;color:#0f766e;">${safeTime}</td></tr>
        </table>
        <p style="margin-top:20px;font-size:14px;color:#374151;">Please ensure your availability at the updated time. If you have any questions, reach us on WhatsApp.</p>
